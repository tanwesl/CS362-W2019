...
int run_tribute(int currentPlayer, int nextPlayer, int *tributeRevealedCards, 
	...
	for (i = 0; i <= 2; i ++){
		...
		//victory Card Found   
				...
				|| tributeRevealedCards[i] == province || tributeRevealedCards[i] == gardens || tributeRevealedCards[i] == great_hall){
			drawCard(currentPlayer, state);
	  		drawCard(currentPlayer, state);
		}
		//Action Card
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		 run_adventurer(currentPlayer, drawntreasure, state, cardDrawn, temphand, z);
      return 0;
    ...
    case mine:
		run_mine(currentPlayer, state, handPos, choice1, choice2, j);
      return 0;
	...
	//****put smithy operation into a function at top of file****//	
    case smithy:
      //+3 Cards
    ...
    case tribute:
		run_tribute(currentPlayer, nextPlayer, tributeRevealedCards, state);
      return 0;
